...
class GaussianDiffusion:
    ...
    def weight_guided_mean(self, args, l, model_out, x, t, classifier, model_kwargs=None):
        """
        Compute the mean for the previous step, given a function cond_fn that
        computes the gradient of a conditional log probability with respect to
        x. In particular, cond_fn computes grad(log(p(y|x))), and we want to
    ...
    def p_sample_loop(
        ...
        if args.guidence_mode == 'weight':
            for sample in self.p_sample_loop_progressive_weight_guided(
                ...
                final = sample
        elif args.guidence_mode == 'output_loss':
    ...
    def p_sample_loop_progressive_weight_guided(
        ...
        #         img = noise.requires_grad_(True)
        #     else:
        #         img = th.randn(*shape, device=device, requires_grad=True)
        if noise is not None:
            img = noise
        else:
